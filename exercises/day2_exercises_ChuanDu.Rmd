...
```{r}
plot(log(gapminder$gdpPercap), log(gapminder$lifeExp), xlab = "GDP per Capita", ylab = "Life Expectancy", main = "Relationship between GDP per capita and life expectancy")
```

Answer: The relationship between GDP per capita and life expectancy is positive, which is, as GDP per capita increases, life expectancy will increase as well.
...
viz_lm <-
  function(year) {
    ...
    fit <- lm(y ~ x)
    plot(y ~ x, main = year,
         xlab = "GDP per capita", ylab = "Life Expentancy")
...
```{r}
for (i in years){
  print(paste0("year:", i, ":", cor(gapminder$lifeExp[gapminder$year == i], gapminder$gdpPercap[gapminder$year == i])))
}
```
...
for (i in (1:nrow(gapminder))){
  if (gapminder[i, "continent"] == "Americas"){
    if (gapminder[i, "country"] == "United States" || gapminder[i, "country"] == "Canada" || gapminder[i, "country"] == "Mexico"){
      gapminder[i, "continent"] = "North America"
    }
    else{
      gapminder[i, "continent"] = "South America"
    }
  }
}
...
viz_lm <-
  function(continent) {
    dat <- gapminder[gapminder[["continent"]] == continent, ]
    y <- log(dat[["lifeExp"]])
    x <- log(dat[["gdpPercap"]])
    fit <- lm(y ~ x)
