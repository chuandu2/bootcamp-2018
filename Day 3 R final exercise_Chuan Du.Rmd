...
Here we set the values of county per poverty to be Low is it is lower than 33% of county per poverty, and set it to be Medium if the value is larger than 33% but less than 66%, and set it to be High if the values if higher than 66%.

```{r}
# Replace lower-than-33% values with "Low"
counties$county_per_poverty[counties$county_per_poverty < quantile(counties$county_per_poverty, 0.33, type = 1, na.rm = T)] <- "Low"

# Replace 33%<=x<66% values with "Medium"
counties$county_per_poverty[counties$county_per_poverty >= quantile(counties$county_per_poverty, 0.33, type = 1, na.rm = T)
      & counties$county_per_poverty <= quantile(counties$county_per_poverty, 0.66, type = 1, na.rm = T)] <- "Medium"

# Replace >=66% values with "High"
counties$county_per_poverty[counties$county_per_poverty > quantile(counties$county_per_poverty, 0.66, type = 1, na.rm = T)] <- "High"
```
